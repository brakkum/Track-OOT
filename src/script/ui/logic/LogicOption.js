...
export default class TrackerLogicOption extends LogicAbstractElement {
    ...
    constructor() {
        ...
        EventBus.register(["state", "settings"], function(event) {
            this.update(event.data[`options.${this.ref}`]);
        }.bind(this));
    }

    update(value) {
        if (typeof value == "undefined") {
            value = StateStorage.read(`options.${this.ref}`, GlobalData.get("settings").options[this.ref].default);
        }
    ...
    static getSVG(logic) {
        ...
        if (!!logic) {
            ...
            let value = StateStorage.read(`options.${logic.el}`, GlobalData.get("settings").options[logic.el].default);
            if (SELECTOR_VALUE.has(this)) {
                value = value == SELECTOR_VALUE.get(this);
            }
...
LogicAbstractElement.registerReference("option", TrackerLogicOption);
